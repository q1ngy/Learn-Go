...
type DemoLocalCodeCache struct {
	cache *lru.Cache
	// 普通锁，或者说写锁
	lock sync.Mutex
	// 读写锁
...
func NewDemoLocalCodeCache(c *lru.Cache, expiration time.Duration) *DemoLocalCodeCache {
	return &DemoLocalCodeCache{
		cache:      c,
		expiration: expiration,
	}
}

func (l *DemoLocalCodeCache) Set(ctx context.Context, biz string, phone string, code string) error {
...
func (l *DemoLocalCodeCache) Verify(ctx context.Context, biz string, phone string, inputCode string) (bool, error) {
	l.lock.Lock()
	defer l.lock.Unlock()
	key := l.key(biz, phone)
	val, ok := l.cache.Get(key)
...
func (l *DemoLocalCodeCache) key(biz string, phone string) string {
